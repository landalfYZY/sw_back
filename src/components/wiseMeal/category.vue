            ...
            <Option v-for="item in shopList" :value="item.sunwouId" :key="item.sunwouId">{{ item.shopName }}</Option>
              ...
              <div class="panel-between">
                <div class="panel-start">
                  ...
                  <button class="ui-btn ui-btn-default" @click="addcates()"><Icon type="plus-round" color="#00CC66"></Icon>&nbsp;&nbsp;添加类目</button>
                </div>
                <div class="panel-end">
...
export default {
  ...
  methods: {
    addcates() {
      if (this.shop == "") {
        this.$Message.error("请先选择一家店铺");
      } else {
        this.catemodel = true;
      }
    },
    submitData() {
      this.formData.shopid = this.shop;
      var that = this;
      if (this.shop == "") {
        this.$Notice.error({
          title: "警告",
