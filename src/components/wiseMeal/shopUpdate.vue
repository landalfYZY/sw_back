  ...
  methods: {
    getShop() {
      ...
      $.ajax({
        ...
        success(res) {
          if (res.code) {
            that.formItem = res.params.list[0];
            that.cityList = res.params.list[0].province;
            that.areaList = res.params.list[0].city;
            var time = [
              res.params.list[0].startTime,
              res.params.list[0].endTime
            ];
            that.formItem.time = time;
          } else {
