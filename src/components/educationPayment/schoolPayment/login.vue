    ...
    <div :style="'width:100%;height:100vh;background-image:url('+bg+');background-size:cover;background-position:center'">
      <div style="width:100%;height:100vh;background:rgba(0,0,0,0.6)">
        ...
        <div style="margin:auto;color:#aaa;width:500px;text-align:Center;margin-top:100px">
          浙江大家地理信息科技有限公司©2017 大家科技 浙ICP备18011227号
        </div>
      </div>
    </div>
  </transition>
              ...
              initGeetest(
                ...
                function (captchaObj) {
                  ...
                  captchaObj.onSuccess(function (res) {
                    ...
                    $.ajax({
                      url: sessionStorage.getItem("API") + "/schoolaccount/login",
                      type: "post",
                      dataType: "json",
                      data: {
                        scAccount: that.scAccount,
                        scKey: that.scKey,
                      ...
                      success(res) {
                        if (res.code) {
                          if (that.loadPass) {
                            ...
                            localStorage.setItem("scAccount", that.data.scAccount);
                            localStorage.setItem("scKey", that.data.scKey);
                          } else {
